...
8. Create the initial pre-release. Tag the template's initial commit as `v0.0.1`:
...
9. Hand off to clean-scaffold. Invoke the `clean-scaffold` skill on `~/Developer/GitHub/<owner>/<name>` so it can scan for placeholders, apply replacements, and rewrite the README.
