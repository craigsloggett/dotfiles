...
arguments:
  - name: consumer-repo
    description: Path to the consumer repo whose dependency references should be bumped.
    required: true
  - name: upstreams
    description: List of upstreams at their new versions, each `{owner/repo, new_tag, tag_sha}`. Supplied by the cascade-change hand-off, or by the user when the releases were cut elsewhere.
    required: true
---
